...
int shiftdiff(unsigned long long int n)
{
	// Difference values
...
int shiftbase(unsigned long long int n)
{
	// Difference values
	int i = 0, diff = 0, pdiff = 0;
...
{
	// Recursion calls
	if (root->west != NULL) if (asmpreproc(fout, cdata, root->west)) return 1;
	if (root->east != NULL) if (asmpreproc(fout, cdata, root->east)) return 1;
	if (root->spex != NULL) if (asmpreproc(fout, cdata, root->spex)) return 1;
	...
	||  (strchr(root->string, '.') != NULL))
	{
		...
		*namext = root->string,
		...
		// Process node string
		strfltok(name, root->string, NULL, "[](){}''\"\"", "~:.", 0);
		namext = root->string;
		...
		for (unsigned int ix = 0, c_off = 0, l = strlen(namext); ix < l;)
		{
			switch (namext[ix])
			{
				case '~':
					// Create node
					ntmp = calloc(1, sizeof(struct offext));
				...
				case ':':
					...
					// Get offset string
					strltok(s_off, namext, NULL, "~:.", c_off);
				...
				case '.':
					...
					root->type = vdatafind(vdatafind(cdata->l_obj, "objname")->list, s_off)->offset;
					root->size = 
					*/
...
int asmprocx64(FILE *fout, struct compiledata *cdata, struct node *troot, struct node *root)
{
	...
	if ((root->west != NULL) && (root->east != NULL))
	{
		...
		*eopename = root->east->east->string,
		...
		unsigned char
		hassetreg, //hassetreg and hadsetreg should be 'hassetreg' 'hadsetreg'
		hadsetreg,
		...
		//printf("CHECK FOR SPILL: '%s %s' %i %i %i\n", root->string, ename, iseop, (!iseoplb && (nodelex(root)/8 == OP_GROUP_EQUAL)), ((iseopwvar || iseopevar || !iseoplb) && !iswvp && !iswprc && (root->west->type != 'p') && (nodelex(root->east) == OP_DIV)));
		
		// Setup push ax
		if (iseop && ((isroot && !iseoplb && (nodelex(root)/8 == OP_GROUP_EQUAL)) || ((iseopwvar || iseopevar || !iseoplb) && !iswvp && !iswprc && (root->west->type != 'p') && (nodelex(root->east) == OP_DIV))))
		{
		...
		if (root->west->size == 0)
		{
			...
			{
				unsigned long long int nsize = stoi(wname);
		...
		if (root->east->size == 0)
		{
			...
			{
				unsigned long long int nsize = stoi(ename);
		...
		if (iswvar)
		{
			if (iswvi) {root->west->ptrlv = vdatafind(cdata->l_i, wname)->ptrlv; root->west->arrsz = vdatafind(cdata->l_i, wname)->arrsz;}
			if (iswvu) {root->west->ptrlv = vdatafind(cdata->l_u, wname)->ptrlv; root->west->arrsz = vdatafind(cdata->l_u, wname)->arrsz;}
			if (iswvp) {root->west->ptrlv = vdatafind(cdata->l_p, wname)->ptrlv; root->west->arrsz = vdatafind(cdata->l_p, wname)->arrsz;}
		}
		if (isevar)
		{
			if (isevi) {root->east->ptrlv = vdatafind(cdata->l_i, ename)->ptrlv; root->east->arrsz = vdatafind(cdata->l_i, ename)->arrsz;}
			if (isevu) {root->east->ptrlv = vdatafind(cdata->l_u, ename)->ptrlv; root->east->arrsz = vdatafind(cdata->l_u, ename)->arrsz;}
			if (isevp) {root->east->ptrlv = vdatafind(cdata->l_p, ename)->ptrlv; root->east->arrsz = vdatafind(cdata->l_p, ename)->arrsz;}
		}
		...
		//do
		{
			...
			if (nspex != NULL)
			{
				...
				// Check if variable had gone to register from the past
				hadsetreg = hassetreg;
				if (!hassetreg) hassetreg = ((root->west->ext != NULL) || (nspex != root->west->spex));
			}
			...
			if (iswvar || iswop)
			{
				if (hadsetreg)
				{
					if (root->west->ext != NULL)
					{
						...
						unsigned int
						c = 0,      	// Pointer level counter
						offset = 0; 	// Setup offset variable (for optimization)
						...
						if (isexf)
						{
							...
							// Check if loop has not run, and is a reference
							if (c == 0 && root->west->isref)
							{
								if (offset > 0) fprintf(fout, "add %s, %i\ncall %s\n", reg_wptr, offset * MAX_BYTESIZE_X64, reg_wptr);
								else            fprintf(fout, "call %s\n",             reg_wptr);
							}
							else
							{
								if (offset > 0) fprintf(fout, "call qword [%s%+i]\n", reg_wptr, offset * MAX_BYTESIZE_X64);
					...
					{
						if (root->west->isref) fprintf(fout, "call %s\n",         wname);
						else                   fprintf(fout, "call qword [%s]\n", wname);
					}
					else if (isexc)
					{
				...
				else
				{
						...
						if (isexf)
						{
							...
							if (c == 0)
							{
								...
								if (root->west->isref)
								{
									if (offset > 0)
									{
										if (iswvs) fprintf(fout, "lea %s, [rbp%+i]\ncall %s\n", reg_wptr, (offset * MAX_BYTESIZE_X64) - vdatafind(cdata->l_s, wname)->offset, reg_wptr);
										else       fprintf(fout, "lea %s, [%s%+i]\ncall %s\n",  reg_wptr, wname, offset * MAX_BYTESIZE_X64, reg_wptr);
									}
									else
									{
										if (iswvs) fprintf(fout, "lea %s, [rbp%+i]\ncall %s\n", reg_wptr, -vdatafind(cdata->l_s, wname)->offset, reg_wptr);
										else       fprintf(fout, "call %s\n",                   wname);
									}
								}
								else
								{
									if (offset > 0)
									{
										if (iswvs) fprintf(fout, "call qword [rbp%+i]\n", (offset * MAX_BYTESIZE_X64) - vdatafind(cdata->l_s, wname)->offset);
										else       fprintf(fout, "call qword [%s%+i]\n",  wname, offset * MAX_BYTESIZE_X64);
									}
									else
									{
					...
					else if (iswvar)
					{
						...
						else if ((root->west->ptrlv > 0) || root->west->arrsz > 1)
						{
							// Force source register to hold pointer
							strcpy(reg_wsrc, reg_wmaxint);
							strcpy(reg_wimm, reg_wmaxint);
		...
		//do
		{
			...
			if (nspex != NULL)
			{
				...
				hadsetreg = hassetreg;
				if (!hassetreg) hassetreg = ((root->east->ext != NULL) || (nspex != root->east->spex));
			}
			...
			if (isevar || iseop)
			{
				if (hadsetreg)
				{
					if (root->east->ext != NULL)
					{
						...
						if (isexf)
						{
							...
							// Store west register if necessary
							if (iswvar || iswop) asmbspillx64_s(fout, cdata, reg_wmaximm, NULL);
							
							// Check if loop has not run, and is a reference
							if (c == 0 && root->east->isref)
							{
								if (offset > 0) fprintf(fout, "add %s, %i\ncall %s\n", reg_eptr, offset * MAX_BYTESIZE_X64, reg_eptr);
								else            fprintf(fout, "call %s\n",             ename);
							}
							else
							{
								if (offset > 0) fprintf(fout, "call qword [%s%+i]\n", reg_eptr, offset * MAX_BYTESIZE_X64);
								else            fprintf(fout, "call qword [%s]\n",    reg_eptr);
							}
							
							// Move result to east register
							fprintf(fout, "mov %s, %s\n", reg_emaxint, reg_wmaxint);
							
							// Load west register if necessary
							if (iswvar || iswop) asmbspillx64_l(fout, cdata, reg_wmaximm, NULL);
						}
						else
						{
							// Update offset value
					...
					else if (isexf)
					{
						...
						if (root->east->isref) fprintf(fout, "call %s\n",         ename);
						else                   fprintf(fout, "call qword [%s]\n", ename);
				...
				else
				{
						...
						if (isexf)
						{
							...
							if (c == 0)
							{
								...
								if (root->east->isref)
								{
									if (offset > 0)
									{
										if (isevs) fprintf(fout, "lea %s, [rbp%+i]\ncall %s\n", reg_eptr, (offset * MAX_BYTESIZE_X64) - vdatafind(cdata->l_s, ename)->offset, reg_eptr);
										else       fprintf(fout, "lea %s, [%s%+i]\ncall %s\n",  reg_eptr, wname, offset * MAX_BYTESIZE_X64, reg_eptr);
									}
									else
									{
										if (isevs) fprintf(fout, "lea %s, [rbp%+i]\ncall %s\n", reg_eptr, -vdatafind(cdata->l_s, ename)->offset, reg_eptr);
										else       fprintf(fout, "call %s\n",                   ename);
									}
								}
								else
								{
									if (offset > 0)
									{
										if (isevs) fprintf(fout, "call qword [rbp%+i]\n", (offset * MAX_BYTESIZE_X64) - vdatafind(cdata->l_s, ename)->offset);
										else       fprintf(fout, "call qword [%s%+i]\n",  ename, offset * MAX_BYTESIZE_X64);
									}
									else
									{
					...
					else if (isevar)
					{
						...
						else if ((root->east->ptrlv > 0) || root->east->arrsz > 1)
						{
							// Force source register to hold pointer
							strcpy(reg_esrc, reg_emaxint);
							strcpy(reg_eimm, reg_emaxint);
	...
	else if (root->root == NULL)
	{
		...
		*name = root->string,
		...
		if (root->size == 0)
		{
			...
			if (isint)
			{
				unsigned long long int nsize = stoi(root->string);
		...
		if (isvar)
		{
			if (isvi) {root->ptrlv = vdatafind(cdata->l_i, name)->ptrlv; root->arrsz = vdatafind(cdata->l_i, name)->arrsz;}
			if (isvu) {root->ptrlv = vdatafind(cdata->l_u, name)->ptrlv; root->arrsz = vdatafind(cdata->l_u, name)->arrsz;}
			if (isvp) {root->ptrlv = vdatafind(cdata->l_p, name)->ptrlv; root->arrsz = vdatafind(cdata->l_p, name)->arrsz;}
		}
		...
		do
		{
			...
			if (nspex != NULL)
			{
				...
				// Check if variable had extension from the past
				hadsetreg = hassetreg;
				if (hassetreg == 0) hassetreg = ((root->ext != NULL) || (nspex != NULL));
			}
			...
			if (isvar)
			{
				...
				else
				{
					...
					else
					{
						...
						// Check if pointer
						else if ((root->ptrlv > 0) || root->arrsz > 1)
						{
							// Force source register to hold pointer
							strcpy(reg_src, reg_maxint);
							strcpy(reg_imm, reg_maxint);
			...
			if (nspex != NULL)
			{
				...
				// Check if still function
				if (nspex->spex != NULL)    isvf = nodeisfun(nspex);
				else
				{
					// Unset function details
					isvf = 0;
					
					// Move to proper pointer
					if (root->ext != NULL && !hadsetreg)
						fputs("mov rdx, rax\n", fout);
				}
...
int callprocx64(FILE *fout, struct compiledata *cdata, struct node *root)
{
	...
	while (strltok(argsl, root->spex->string, NULL, ",", argc))
		argc++;
	...
	{
		strltok(argsl, root->spex->string, NULL, "(,)", i);
		strstrip(argsl);
	...
	*droot = NULL;  	// Root of destination node
	...
	else for (unsigned int i = 0; strfltok(token[c_tok], args, "~:.", "[](){}''\"\"", " \t", i); ++i, ++c_tok)
	{
		switch (c_tok)
		{
			...
			case 0:
				...
				while (strfltok(specl[0], args, "~:.", "[](){}''\"\"", " \t", i + 1))
				{
					struct node *n;
					switch (specl[0][0])
					{
			...
			case 2:
				...
				// Check for special extensions
				while (strfltok(specl[1], args, "~:.", "[](){}''\"\"", " \t", i + 1))
				{
					struct node *n;
					switch (specl[1][0])
					{
	...
	if (root != NULL)
	{
		...
		if (droot == NULL)
		{
			...
			{
				fputs("\nError: Assembly generation has failed.\n---\n", stderr);
				if (autofree && (root != NULL)) nodedest(root);
				return NULL;
			}
		}
...
// Check for keywords
int kwordproc(struct compiledata *cdata, FILE *fout, char *args)
{
	...
	if (!strcmp(token[0], "if"))
	{
		...
		if (cdata->c_condlv < MAX_NESTLEVEL_CODE)
		{
	...
	if (!strcmp(token[0], "case"))
	{
		...
		// Write condition
		cdata->c_condmin[cdata->c_condlv - 1]++;
		fprintf(fout, "cmp rax, %s\njne .c%i_%i\n", token[1], cdata->c_condmaj[cdata->c_condlv - 1], cdata->c_condmin[cdata->c_condlv - 1]);
	}
	if (!strcmp(token[0], "nocase"))
	{
		// Write jump label (related to previous comparison code)
		fprintf(fout, ".c%i_%i:\n", cdata->c_condmaj[cdata->c_condlv - 1], cdata->c_condmin[cdata->c_condlv - 1]);
	...
	if (cdata->f_infunc == 0) 
	{
		if (!strcmp(token[0], "fn"))
		{
			fprintf(fout, "%s:\n", token[1]);
			
			// Assign as function in list
			vdatainit(cdata->l_f, token[1], 0, 0, 0, 0, 0);
			
			// Change file code pointer
			fmodproc(cdata, fout, 1);
		}
		if (!strcmp(token[0], "nt"))
		{
				...
				if (cdata->c_stacklv > 0)
				{
					// x86-64 assembly code
						...
						switch (cdata->f_abi)
						{
							...
							default:
								fputs("\nError: You may have broken something in the compiler's source code.\nDebug 'main.c', in function 'kwordproc'.\n---\n", stderr);
								return 1;
						}
	...
	if (!strcmp(token[0], "lb"))
	{
		// Create location label
		fprintf(fout, "%s:\n", token[1]);
	}
	if (!strcmp(token[0], "gt"))
	{
		// Jump to location
		fprintf(fout, "jmp %s\n", token[1]);
	}
	if (!strcmp(token[0], "asm"))
	{
		fprintf(fout, "%s\n", stritok(args, "''\"\"", " \t,", 1));
	}
...
int vdataproc(FILE *fout, struct vlist *l_type, char *args, char *name, int type, int cvsize, int dupsz)
{
	...
	token[STRING_LENGTH_TOKEN] = "";
	
	// Loop to print data stuff to file
	for (unsigned int i = 0; strfltok(token, args, NULL, "''\"\"", ",", i); ++i)
	{
		if (i > 0) strcat(line, ",");
	...
	// Set array size in vdata
	vdatafind(l_type, name)->arrsz = arrsz;
...
// Define a variable with type and size
// TODO: The function must return the number of tokens used
int vdefproc(struct compiledata *cdata, FILE *fdata, FILE *fcode, char *args)
{
	int 
	type = 0,
	size = 0;
	...
	// Initialize token
	char token[STRING_LENGTH_TOKEN];
	strltok(token, args, "''\"\"", " \t", 0);
	...
	if (strlen(token) >= 2)
	{
		...
		if ((type > 0) && (size > 0))
		{
			...
			for (unsigned int i = 1; strltok(token, args, "''\"\"", " \t", i); ++i)
			{
				...
				if (stage == 1)
				{
					...
					switch (token[0])
					{
						...
						case '.':
							...
							return 1;
						default:
							vdatainit(l_type, token, type, size, ptrlv, 0, arrsz);
							break;
					}
					...
					if (cdata->f_infunc)
						vdatainit(cdata->l_s, token, type, size, ptrlv, cdata->c_stacklv, arrsz);
			...
			// (VERBOSE) Show assigned variable data
			if (cdata->f_verbose)
			{
				...
				switch (type)
				{
					case 'i': printf("signed ");   break;
					case 'u': printf("unsigned "); break;
					case 'p': printf("precise ");  break;
				}
				switch (size)
				{
					case 1: printf("byte");  break;
					case 2: printf("word");  break;
					case 4: printf("dword"); break;
					case 8: printf("qword"); break;
				}
	...
	else
	{
		...
		while (fgets(line, STRING_LENGTH_LINE, fptr))
		{
			...
			if (strlen(line))
			{
				...
				// Parse the line
				if ((vdefproc(cdata, fdata, fcode, line)) 	// Variable definition
				|| (kwordproc(cdata, fcode, line)))       	// Conditional statements
					return 1;
			}
		}
...
int fcompile(struct compiledata *cdata, FILE *fout, FILE *fdata, FILE *fcode)
{
	...
	switch (cdata->f_abi)
	{
		...
		case OS_WIN32: fprintf(fout, "format PE64\n"); break;
	}
	...
	{
		fclose(fdata);
		if (!cdata->f_noparse) remove("data.txt");
	}
	...
	{
		fclose(fcode);
		if (!cdata->f_noparse) remove("codex.txt");
	}
	...
	*value = stritok(args, NULL, "=",  1);
	...
	switch (args[1])
	{
		case '-':
			...
			if (strstr(flag, "noparse")  == args + 2) goto j_noparse;  else
		...
		case 'h':
		j_help:
			printf("Verse Compiler (v%u.%u.%u)\n", VERSION_MAJOR, VERSION_MINOR, VERSION_PATCH);
			puts("Copyright (c) 2022, chris03-dev\n");
			
			puts("Usage: verse <output file> <flag=value> ... <input file> ...\n");
			...
			puts("-vso*  1 = Enable variable substitution to    0 - 2");
			puts("           optimize operations.");
			puts("       2 = Enable optimization loop and");
...
int main(int argc, char **argv)
{
	...
	/*
	char 
	c[] = "hello World(lol)~203184 Hi~(hello)",
	ci[32];
	
	for (unsigned int i = 0; strfltok(ci, c, "~", "()", " ", i); ++i)
		printf("TOKEN %i: %s\n", i, ci);
	*/
	...
	*fdata = fopen("data.txt",  "a+");
	...
	{
		fprintf(stderr, "\nError: '%s' does not have permission to generate files.\n---\n", argv[0]);
		return 1;
	}
	...
	if (fout != NULL)
	{
		...
		// Free lists
		vdatadestall(cdata.l_f);
		vdatadestall(cdata.l_v);
		vdatadestall(cdata.l_c);
